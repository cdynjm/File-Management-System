...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 " id="sidenav-main">
  <div class="sidenav-header">
    ...
    <a class="align-items-center d-flex m-0" href="{{ route('dashboard') }}">
        ...
        @if(Auth::user()->type == 0)
          @foreach(Logo::where(['admin_id' => Auth::user()->admin_id])->get() as $logo)
            @if($logo->logo != null)
        ...
        @if(Auth::user()->type == 1)
          @foreach(Logo::where(['admin_id' => Auth::user()->id])->get() as $logo)
            @if($logo->logo != null)
                <img style="width: 70px; height: 70px; border-radius: 50px;" src="{{ asset('storage/logo/'.$logo->logo) }}" class="ms-4" alt="...">
            @endif
            @if($logo->logo == null)
  ...
  <div class="collapse navbar-collapse  w-auto h-auto" id="sidenav-collapse-main">
    <ul class="navbar-nav">
      ...
      @endif
      <li class="nav-item pb-2">
        <a class="nav-link {{ (Request::is('SB-Members') ? 'active' : '') }}" href="{{ url('SB-Members') }}">
            <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                <svg class="text-dark {{ (Request::is('SB-Members') ? 'text-white' : 'text-dark') }} "  xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-people-fill" viewBox="0 0 16 16">
                  <path d="M7 14s-1 0-1-1 1-4 5-4 5 3 5 4-1 1-1 1H7Zm4-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm-5.784 6A2.238 2.238 0 0 1 5 13c0-1.355.68-2.75 1.936-3.72A6.325 6.325 0 0 0 5 9c-4 0-5 3-5 4s1 1 1 1h4.216ZM4.5 8a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5Z"/>
                </svg>
            </div>
            <span class="nav-link-text ms-1">SB Members</span>
        </a>
      </li>
      <li class="nav-item mt-2">
