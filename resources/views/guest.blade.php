  ...
  <main class="main-content  mt-0">
    <section>
      <div class="page-header min-vh-75">
        ...
        <h6>Recently Approved Ordinances</h6>
            <div class="row">
            @foreach ($recent_files as $fil)
                ...
                <div class="card">
                  <div class="card-header mx-4 p-3 text-center">
                        ...
                        <div class="icon icon-shape icon-lg bg-gradient-info shadow text-center border-radius-lg">
                            <i class="fas fa-file-word opacity-10"></i>
                        @endif
                        ...
                        <div class="icon icon-shape icon-lg bg-gradient-success shadow text-center border-radius-lg">
                            <i class="fas fa-file-excel opacity-10"></i>
                        @endif
                  ...
                  <div class="card-body pt-0 p-3 text-center">
                    ...
                    <h6 class="text-center mb-0">{{ $fil->ordinance }}</h6>
                    
                    <span class="text-xs">{{ date('F d, Y', strtotime($fil->date_approved)) }}</span>
                    <span class="text-xs"> - {{ $fil->ord_number }}</span>
                    <hr class="horizontal dark my-3">
                    <p class="mb-0" style="font-size: 13px;">{{ $fil->description }}</p>
                  </div>
                </div>
                </a>
              </div>
            ...
            <div class="col-12">
            <div class="card-body p-2 mt-4 bg-white">
                    <p class="m-4">All Approved Ordinances</p>
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0">
                            ...
                            @foreach ($files as $fil)
                                <tr>                                  
                                    <td class="ps-4" fileid='{{ $fil->id }}'>
                                        <p class="text-xs font-weight-bold mb-0">{{ $cnt }}</p>
                                    </td>
                                    <td class="text-left" ordinance='{{ $fil->ordinance }}'>
                                      <a href="{{ asset('storage/files/'.$fil->filename) }}" target="blank">
                                    ...
                                    <td class="text-center" ord_number='{{ $fil->ord_number }}'>
                                        <p class="text-xs font-weight-bold mb-0">{{ $fil->ord_number }}</p>
                                    </td>
                                    <td class="text-center" author='{{ $fil->author }}'>
                                        <p class="text-xs font-weight-bold mb-0">{{ $fil->Author->name }}</p>
                                    </td>
                                    <td class="text-center" coauthor='{{ $fil->co_author }}'>
                                        <p class="text-xs font-weight-bold mb-0">{{ $fil->CoAuthor->name }}</p>
                                    </td>
                                    <td class="text-center" description='{{ $fil->description }}'>
                                        <p class="text-xs font-weight-bold mb-0">{{ $fil->description }}</p>
                                    </td>
                                    <td class="text-center" dateapproved='{{ $fil->date_approved }}'>
                                        <p class="text-xs font-weight-bold mb-0">{{ date('F d, Y', strtotime($fil->date_approved)) }}</p>
                                    </td>
                                    <td class="text-center" category='{{ $fil->Category->category }}'>
